...
df_dog <- df_dog %>%
  ...
  dplyr::mutate(
    acquisition_source = forcats::fct_collapse(
      acquisition_source,
      #rescue=c("found"),
      pet_shop=c("online")
    )
  ...
  dplyr::mutate(
    revised_acquisition_source = forcats::fct_collapse(
      revised_acquisition_source,
      #rescue=c("found"),
      pet_shop=c("online")
    )
...
```{r}
df_owner <- df_owner %>%
  mutate(owner_gender = as.factor(owner_gender)) %>%
  mutate(owner_age = as.numeric(owner_age)) %>%
  mutate(datetime = as.POSIXct(strptime(datetime,"%m/%d/%Y %H:%M:%S"))) %>%
...
for (rank_col in char_map) { 
  ...
  for (r in 1:nrow(df)) {
    df[r, revised_rank_col] <- ifelse(
      df[r, "is_consider_another_dog"] == TRUE, 7, NA)
  }
}
...
df <- df %>%
  ...
  mutate(is_owner_male = (owner_gender == "male"))
```


## Inclusion Critera

Count the number of owners before applying inclusion criteria.

```{r}
length(unique(df$owner_id))
```

Count the number of responses before applying inclusion criteria.
...
df <- df %>%
  filter(owner_age >= 18)
stopifnot(identical(dim(df)+0, c(1543, 29))) # Verify expected dim.
```
...
```{r}
# rank_cols <- colnames(
#   df %>% dplyr::select(contains("rank") & !contains("revised")))
# 
...
#   dplyr::select(-is_rank_valid)
# stopifnot(identical(dim(df)+0, c(1411, 28))) # Verify 132 entries dropped.
```

### Drop Entries Missing Required Data

```{r}
df <- df %>%
  filter(!is.na(owner_gender))
stopifnot(identical(dim(df)+0, c(1537, 29))) # Verify expected dim.
```
...
duplicates <- df %>%
  dplyr::count(owner_id, dog_name) %>%
  filter(n>1)
stopifnot(identical(dim(duplicates)+0, c(0, 3))) # Verify no duplicates.
```
